...
const MoviePage = () => {
  ...
  const submit = async () => {
    const uniqueTicketId = generateRandomString(hourIndex);
    const requestData = {
      userId: getCurrentUserID(),
      //modify hereeeeeeeeeeeeeee
      sansId: movieDates[dateIndex].id,
      //modify hereeeeeeeeeeeeeee
      seats: chosenTickets,
      ticketId: uniqueTicketId,
    };
    try {
      const currentMovie = await createBooking(requestData);
      window.location = `/result/${uniqueTicketId}`;
    } catch (err) {}
  };
